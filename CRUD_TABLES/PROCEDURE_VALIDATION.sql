...
CREATE OR REPLACE FUNCTION valida_img_url (
...
CREATE OR REPLACE FUNCTION valida_id (
    p_id IN NUMBER
) RETURN BOOLEAN
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE INSERT_PROCEDURE_VALIDATION(
...
BEGIN
    ...
    WHERE ID = p_procedure_status_id;
...
CREATE OR REPLACE PROCEDURE UPDATE_PROCEDURE_VALIDATION(
    p_id IN NUMBER,
    p_img_url_initial IN VARCHAR2,
    p_img_url_final IN VARCHAR2,
    p_procedure_type_id IN NUMBER,
...
CREATE OR REPLACE PROCEDURE DELETE_PROCEDURE_VALIDATION(
    p_id IN NUMBER
)
IS
    v_count NUMBER;
...
EXEC INSERT_PROCEDURE_VALIDATION('http://example.com/initial.jpg', 'http://example.com/final.jpg', 1, 1);

EXEC READ_PROCEDURE_VALIDATION(7);
